...
- Let \(n_k\) be the size of cluster \(C_k\), for each
  \(k=1,2,\ldots,K\)
...
- Remove from \(C_k\) the elements in
...
- From each \(C_k\), sample with replacement \(m\) items to fill

\[
C^{*}_{bk}, \qquad k=0,1,\ldots,K
\]
...
- Collect the Wald's test statistics \(W_{bkj}\) for each gene
  ...
  where \(p\) is the number of genes, and for each
  \(k^{\mathrm{th}}\) group, with \(k=1,2,\ldots,K\).

---

Repeat Steps 1, 2, and 3 for \(B\) bootstrap iterations
(\(b=1,2,\ldots,B\)).
